<nav class="navbar navbar-expand-lg bg-success bg-gradient mt-3 ">
  <div class="container-fluid ">
    <a class="navbar-brand" href="#">Dashboard</a>
    <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
      <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarNav">
      <ul class="navbar-nav">
        <li class="nav-item">
          <a class="nav-link text-light" aria-current="page" href="index.php">Customer</a>
        </li>
        <li class="nav-item">
          <a class="nav-link text-light" href="transaction.php">Transactions</a>
        </li>
        <li class="nav-item">
          <a class="nav-link text-light" href="product.php">Products</a>
        </li>
        <li class="nav-item">
          <a class="nav-link text-light" href="supplier.php">Suppliers</a>
        </li>
        <li class="nav-item">
          <a class="nav-link text-light" href="stock.php">Stock</a>
        </li>
              <li class="nav-item">
          <a class="nav-link text-light" href="charts.php">Charts</a>
        </li>
        <!-- <li class="nav-item">
          <a class="nav-link disabled" aria-disabled="true">Disabled</a>
        </li> -->
      </ul>
    </div>
  </div>
</nav>